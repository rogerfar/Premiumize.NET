...
$binDirectoryPath = ".\PremiumizeNET\bin"
if (Test-Path $binDirectoryPath) {
    Remove-Item -Path $binDirectoryPath -Recurse -Force
}

$csprojPath = Join-Path -Path $scriptPath -ChildPath "PremiumizeNET\PremiumizeNET.csproj"
...
if (Test-Path $csprojPath) {
    ...
    Write-Error "PremiumizeNET.csproj file not found."
	exit
}

dotnet publish ".\PremiumizeNET" -c Release
...
$packageFilePath = Get-ChildItem -Path ".\PremiumizeNET\bin\Release" -Filter "*.nupkg" | Select-Object -First 1
